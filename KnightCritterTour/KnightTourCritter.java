...
public class KnightTourCritter extends KnightCritter
{
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        for ( int i = 0; i < locs.size(); i++ )
        {
            ...
            if ( accessGrid[row][col] < min )
            {
                min = accessGrid[row][col];
                bestMove = locs.get( i );
            }
        }
        return bestMove;
    ...
    {
        ArrayList<Location> l = getPossibleMoves( loc );
    ...
    {
        for ( int i = 0; i < accessGrid.length; i++ )
        {
            for ( int j = 0; j < accessGrid[0].length; j++ )
            {
                System.out.print( "[" + accessGrid[i][j] + "]" );
            }
            System.out.println();
        }
        System.out.println();
