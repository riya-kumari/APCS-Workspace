...
public class Actor implements Comparable<Actor>
{
    ...
    public boolean receiveItem( Actor giver, String item )
    {
        if ( !myPossessions.contains( item ) )
        {
            ...
            email.add(
                new Message( this, giver, "thanks for the " + item ) );
            return true;
        }
        else
            return false;
    ...
    //
    // one.
    // Processing messages:i)
    //
    // Takes action only for
    //
    // messages with
    //
    // the text"need <item>","have <item>",or"ship <item>".
    //
    // where<item> is
    //
    // the name
    ...
    // of the item,
    //
    // such as"hat","left glove",or"right glove".
    //
    // Skips all
    //
    // other messages.ii)If this
    //
    // is a"need"
    //
    // message and this
    ...
    // item requested, it
    //
    // replies with a"have <item>"message.iii)If this
    //
    // is a"have"
    //
    // message and this
    ...
    // offered item, it
    //
    // replies with a"ship <item>"message.iv)If this
    //
    // is a"ship"
    //
    // message and this
    ...
    // receiveItem method.
    //
    // If receiveItem returns true,
    //
    // removes item from this actor'spossessions.
    //
    // 3.
    //
    // If allSetFlag
    ...
    // one of
    //
    // each item, sends"thanks, i'm all set"
    //
    // to the list.
    ...
    public void readMail()
    {
        ...
        while ( !mailbox.isEmpty() )
        {
            ...
            String theItem = msg.getText().substring( 5 );
            boolean extraItem = false;

            if ( countPossessions( theItem ) > 1 )
                extraItem = true;

            if ( text.contains( "need" ) )
            {
                if ( extraItem == true )
                    send( msg.getSender(), "have " + theItem );
            }
            else if ( text.contains( "have" ) )
            {
                if ( extraItem == false )
                    send( msg.getSender(), "ship " + theItem );
            ...
            else if ( text.contains( "ship" ) )
            {
                if ( extraItem == true )
                    if ( msg.getSender()
                        .receiveItem( msg.getRecipient(),
                            theItem ) == true )
                        myPossessions.remove( theItem );
